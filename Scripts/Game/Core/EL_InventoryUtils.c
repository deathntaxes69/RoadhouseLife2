class RL_InventoryUtils
{
	...
	static IEntity GetStorageHierachyRoot(IEntity item)
	{
		while (item)
		{
			//Print("[RL_InventoryUtils] GetStorageHierachyRoot while loop");
			InventoryItemComponent itemComponent = InventoryItemComponent.Cast(item.FindComponent(InventoryItemComponent));
	...
	static InventoryStorageManagerComponent GetResponsibleStorageManager(IEntity item)
	{
		while (item)
		{
			//Print("[RL_InventoryUtils] GetResponsibleStorageManager while loop");
	...
	//------------------------------------------------------------------------------------------------
	static array<IEntity> FindItemsByPrefab(notnull InventoryStorageManagerComponent storageManager, ResourceName prefab, bool includeEquipped = false)
	{
		...
		if (includeEquipped)
			purpose = EStoragePurpose.PURPOSE_ANY;
		else
			purpose = EStoragePurpose.PURPOSE_DEPOSIT;
		
		storageManager.FindItems(foundItems, prefabNamePredicate, purpose);
		return foundItems;
	}
}
